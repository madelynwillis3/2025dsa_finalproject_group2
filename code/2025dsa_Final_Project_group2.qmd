...
library(stringr)
# Step 1: Create a lookup table of lat/lon for non-missing coordinates by actual site
coord_lookup <- df_full %>%
  filter(!is.na(latitude) & !is.na(longitude)) %>%
  group_by(site.x) %>%
  summarise(lat_site = mean(latitude), lon_site = mean(longitude), .groups = "drop")

# Step 2: Left join to bring coordinates to matching NAs
df_full <- df_full %>%
...
df_model <- df_full %>%
  select(-yield_mg_ha, -replicate, -block, -year.x, -site.x, -site_year) # keep year_site

# double check your target is still there!
glimpse(df_model)

yield_rec <- recipe(yield_mg_ha_adj ~ ., data = df_model) %>%
  step_zv(all_predictors()) %>%
  step_integer(all_nominal_predictors()) %>%  # <-- better for high-cardinality
...
boost_spec <- boost_tree(
  trees = 100,          # lower number of trees
  tree_depth = 3,       # shallower trees
  learn_rate = 0.1,     # standard learning rate
  loss_reduction = 0,
  sample_size = 0.8,
  mtry = 0.5
) %>%
...
xgb_wf <- workflow() %>%
  add_recipe(yield_rec) %>%
  add_model(xgb_spec)

folds <- vfold_cv(df_full, v = 5)

doParallel::registerDoParallel()

xgb_results <- tune_race_anova(
  xgb_wf,
  resamples = folds,
  grid = 20,
...
best_params <- select_best(xgb_results, metric = "rmse")

final_xgb <- finalize_workflow(xgb_wf, best_params)

final_fit <- final_xgb %>% 
  last_fit(split = initial_split(df_full, prop = 0.8))
