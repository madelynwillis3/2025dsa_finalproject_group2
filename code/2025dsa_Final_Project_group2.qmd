...
rf_grid_result
# Let's find our number of trees from the lowest RMSE and highest R2
## Based on RMSE
rf_grid_result %>%
  collect_metrics() %>%
  filter(.metric == "rmse") %>%
...
```{r Random Forest final_spec}
final_spec_rf <- rand_forest(trees = 443,
                          mtry = 25) %>%
  # Specify the engine
  set_engine("ranger",
...
```{r Random Forest fit on test set}
final_fit_rf %>%
  collect_metrics()
```
...
```{r - Permutation evaluation}
df_prep <- df_recipe %>%
  prep()
final_spec %>%
  fit(yield_mg_ha_adj ~ .,
         data = bake(df_prep, df_model)) %>%
    vi() %>%
  mutate(
