...
=======
cat_cols <- df_model %>%
>>>>>>> df1b0fb07625f1fde66123f24546054e0630f883
...
# Setup parallel backend in case XGBoost uses it
library(doParallel)
registerDoParallel(cores = 2)

# Optional: clean rare levels again on full dataset
cat_cols <- df_model %>%
  select(where(is.factor)) %>%
  names()
...
full_recipe <- recipe(yield_mg_ha ~ ., data = df_model) %>%
  step_nzv(all_predictors()) %>%
  step_impute_median(all_numeric_predictors()) %>%
  step_unknown(all_nominal_predictors()) %>%        # <-- NEW
  step_dummy(all_nominal_predictors(), one_hot = TRUE)

summary(df_model)  # check for NAs
sapply(df_model, function(x) sum(is.na(x)))  # quick NA count


# Default model spec — you can tweak these if you want
xgb_default_spec <- boost_tree(
  trees = 100,
  tree_depth = 5,
  min_n = 10,
  learn_rate = 0.1
...
xgb_default_workflow <- workflow() %>%
  add_recipe(full_recipe) %>%
  add_model(xgb_default_spec)
registerDoParallel(cores = parallel::detectCores()-1)
# ⏱️ Time it
tic("Fitting XGBoost on full data (no tuning)")
xgb_default_fit <- fit(xgb_default_workflow, data = df_model)
toc()
...
# Step 2: Add predictions to the `baked_test_data
baked_test_data <- baked_test_data  %>%
